...
import vcart from '../assets/vcart.png'
...
function Nav() {
  ...
  return (
    <div className='w-[100vw] lg:h-[70px] md:h-[50px]  bg-[#ecfaec] z-10 fixed top-0 flex items-center justify-between px-[30px] shadow-md shadow-black  ' >
      ...
      <div className='w-[30%] flex items-center justify-end gap-[20px]  ' >
        {!showSearch && <IoSearchCircleOutline className='w-[30px] h-[38px] text-[#000000] cursor-pointer ' onClick={() => { setShowSearch(prev => !prev); navigate("/collection") }} />}
        {showSearch && <IoSearchCircle className='w-[30px] h-[38px] text-[#000000] cursor-pointer ' onClick={() => setShowSearch(prev => !prev)} />}
        {!userData && <FaUserCircle className='w-[27px] h-[27px] text-[#000000] cursor-pointer ' onClick={() => setShowProfile(prev => !prev)} />}
        ...
        <IoCart className='w-[30px] h-[38px] text-[#000000] cursor-pointer hidden md:block  ' onClick={() => navigate("/cart")} />
      ...
      {showProfile && <div className='absolute w-[220px] h-[150px] bg-[#000000d7] top-[110%] right-[4%] border-[1px] border-[#aaa9a9] rounded-[10px] z-10 ' >
        <ul className='w-100% h-[100% flex items-start justify-around flex-col text-[17px] py-[10px] text-[white] '>
          ...
          {userData && <li className='w-[100%] hover:bg-[#2f2f2f] px-[15px] py-[10px] cursor-pointer ' onClick={() => { handleLogout("/login"); setShowProfile(false) }}>Logout</li>}
      ...
      <div className='w-[100vw] h-[90px] flex items-center justify-between px-[20px] fixed bottom-0 left-0 bg-[#191818] md:hidden text-[12px] '>
        ...
        <button className='text-[white] flex items-center justify-center flex-col gap-[2px] '><HiOutlineCollection className='w-[28px] h-[28px] text-[white] md:hidden' onClick={() => navigate("/collection")} /> Collection</button>
        ...
        <button className='text-[white] flex items-center justify-center flex-col gap-[2px] '><IoCart className='w-[28px] h-[28px] text-[white] md:hidden ' onClick={() => navigate("/cart")} />Cart</button>
